...
import referralImg from "../../assets/images/concept-of-online-referral-program.png";
import { AuthContext } from "../../context/AuthContext";
const Referral = () => {
  const { user } = useContext(AuthContext);
  const domain = window.location.origin;
  const refLink = useState(`${domain}/ref=${user?.username}`)[0];
  const [copyState, setCopyState] = useState(false);
  return (
    <div className='w-full rounded-lg flex flex-col '>
      <div className='text-white flex flex-row bg-yellow-500 p-2 lg:rounded-t-lg space-x-2 items-center'>
        <span>
          <svg
      ...
      <div className='p-4 space-y-4'>
        <span className='text-lg lg:pr-10 pl-5 flex justify-between'>
          <span className='italic'>You have not referred anyone yet</span>
        ...
        <form className='flex flex-row w-full'>
          ...
          <button
            type='button'
            className='py-2 px-2 bg-yellow-500 text-sm capitalize rounded-r-lg'
            onClick={() => {
